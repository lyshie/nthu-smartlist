...
# lyshie_20120314: load the priority table for pretty sorting
sub getUnitOrders
{
    my $filename_order = "$Bin/unit/order";

    return if (!-f $filename_order);

    open(FH, $filename_order);
    while (<FH>) {
        my $line = $_;
        chomp($line);
        my ($name, $parent, $unit, $type, $digest) = split(/:/, $line);
        my %hash = (
            'parent' => $parent,
            'unit'   => $unit,
            'type'   => $type,
        ...
        $UNIT_ORDERS{$name} = \%hash;
    }
    close(FH);
}

sub getSubscribeContent
{
    my $result = <<EOF
<h2>電子報訂閱結果</h2>
<br />
...
;
    ...
    if ($pop3) {
        foreach my $s (@subscriptions) {
            foreach my $list (@lists) {
                if ($s eq $list) {
                    ...
                    $result .= "直接訂閱 [$list] $fields{'DESC'}\n";
                }
            }
        }
    }
    else {
        $result .= "已寄出確認信至您的信箱，欲訂閱的電子報清單如下：\n";
        ...
        chomp($list_msg);
        $subscribe_part =~ s/#IP#/$REMOTE_ADDR/gm;
        $subscribe_part =~ s/#LISTS#/$list_msg/gm;
        $subscribe_part =~ s/#UID#/s$UID/gm;
        $subscribe_part =~ s/#EMAIL#/$email/gm;
...
To: $to
Subject: subscribe confirm s$UID
$subscribe_part
EOF
;
        close(FH);
...
<br />
<div align="center">
<a href="subscribe.cgi?sid=$sid">回上一頁</a>
</div>
EOF
;
...
<table class="light" align="center" width="100%">
	<tr class="listheader">
		<td>簡易訂閱(直接訂閱)</td>
		<td>一般訂閱(<span style="color: yellow;">需點選確認</span>)</td>
	</tr>
	<tr class="list">
		<td style="text-align: justify;">本訂閱方式採用本中心提供的電子郵件信箱其密碼來驗證。[<a href="http://net.nthu.edu.tw/2009/faq:mailing_paper" target="_blank">詳細說明</a>]</td>
		<td style="text-align: justify;">如果你的信箱為 Yahoo Mail、Gmail、Hotmail 等非本中心提供的電子郵件信箱，只能採用本方式來訂閱電子報，但本訂閱方式不限信箱種類(含本中心提供的電子郵件信箱)皆可使用。[<a href="http://net.nthu.edu.tw/2009/faq:mailing_paper" target="_blank">詳細說明</a>]</td> 
	</tr>
	<tr class="list">
		<td>
<!-- subscribe -->
...
<tr>
	<td class="light" colspan="2" style="text-align: center;">
		<input id="ds" type="submit" name="action" value="簡易訂閱" />
	</td>
</tr>
</table>
<!-- subscribe -->
		...
		<td>
			...
			<input id="cs" type="submit" name="action" value="一般訂閱" />
		</td>
	</tr>
</table>
<table class="light" align="center" width="100%">
...
;
    getUnitOrders();
    my ($parent, $type, $digest) = ('', '');

    my @lists = getListNames();
    my $index = 0;
    foreach my $list (
        sort { $UNIT_ORDERS{$a}->{'type'} cmp $UNIT_ORDERS{$b}->{'type'}     ||
               $UNIT_ORDERS{$a}->{'parent'} cmp $UNIT_ORDERS{$b}->{'parent'} ||
               $a cmp $b }
        @lists)
    {
        if ($parent ne $UNIT_ORDERS{$list}->{'parent'}) {

            $parent = $UNIT_ORDERS{$list}->{'parent'} || '其他';
            $type   = $UNIT_ORDERS{$list}->{'type'}   || '其他';
            $digest = $UNIT_ORDERS{$list}->{'digest'} || 'other';

            $result .= <<EOF
	<tr class="list">
		<th>
			<input type="button" value="選取" onclick="CheckGroup(this.form, '$digest');" />
		</th>
		<th colspan="3">
			$parent ($type)&nbsp;
		</th>
...
;
	...
	<tr class="list color$style" id="tr_$fields{'NAME'}">
		...
		<td><input type="checkbox" name="lists" value="$fields{'NAME'}" class="$digest" id="checkbox_$fields{'NAME'}" /></td>
		<td>
...
sub main
{
    ...
    if (($action ne '') && ($email ne '') && (@subscriptions > 0)) {
        ...
        print templateReplace('subscribe_sorted.ht',
        ...
        print templateReplace('subscribe_sorted.ht',
                              {'TITLE'    => getDefaultTitle(),
                               ...
                               'WIDGET_2' => getWidgetLatestArticles(),
                               'WIDGET_3' => getNull(),
                               'CONTENT'  => getSubscribeFormContent(),
                              }
                             );
    }
}
