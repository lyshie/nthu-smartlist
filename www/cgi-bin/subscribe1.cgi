        ...
        foreach my $list (@lists) {
            if ($s eq $list) {
                ...
                if ($pop3) {
                    ...
                    $result .= "直接訂閱 [$list] $fields{'DESC'}<br />\n";
                    next;
                }
...
Subscribe from $REMOTE_ADDR ($REMOTE_HOST),
at $REQUEST_URI
EOF
;
                close(FH);
                $result .= "欲訂閱 [$list] $fields{'DESC'}，等候確認信回覆<br />\n";
                last;
            }
        }
    }
...
<table class="light" align="center" width="100%">
	<tr class="listheader">
		<td>簡易訂閱(直接訂閱)</td>
		<td>一般訂閱(<span style="color: yellow;">需回信確認</span>)</td>
	</tr>
	<tr class="list">
