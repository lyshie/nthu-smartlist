...
my $FILE_BAR   = "$SMARTLIST_PATH/www/htdocs/images/graph/bar_this_year.png";
...
sub getListData
{
    ...
    $GRAPH_DATA = GD::Graph::Data->new([\@months, \@a_counts, \@p_counts
...
{
    getListData();
    $GRAPH = GD::Graph::hbars->new(680, 360);

    my $x_label = reverse Encode::decode('utf-8', '月');

    $GRAPH->set(
        x_label         => $x_label,
        y_label         => '電子報數量',
        title           => ($year + 1900) . ' 年總發行統計',
        y_plot_values   => 1,
        x_plot_values   => 1,
        # shadows
        shadow_depth    => 1,
