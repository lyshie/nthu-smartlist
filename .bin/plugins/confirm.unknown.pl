...
my $SENDMAIL    = "/usr/sbin/sendmail -oi -t -f $LISTNAME-request\@" . getListDomain();
...
;
    ...
    syslog(LOG_INFO,
           "Already on the subscriber list [%s - %s]",
           $LISTNAME,
           $SENDER
          );
}
...
;
    ...
    syslog(LOG_INFO,
           "Your name is not on the list [%s - %s]",
           $LISTNAME,
           $SENDER
          );
}
...
sub parseCommand
{
    ...
    if ($XLOOP eq "$LISTNAME-request\@" . getListDomain()) {
        syslog(LOG_INFO,
               "Mail loop detected! Drop it! [%s - %s]",
               $LISTNAME,
               $XLOOP,
              );
        return;
    ...
    if (checkUndelivered($SENDER, $subject, '')) {
        ...
        addToBounce($LISTNAME, \@emails); 
            ...
            open(FH, ">>/usr/local/slist/tmp/undelivered") or die($!);
            print FH "----------------------------------------------------------\n";
            print FH $MAIL_HEADER->as_string(), "\n";
            foreach (@$MAIL_BODY) {
            print FH "$_";
